...
namespace Free.FileFormats.VRML.Nodes
{
	...
	/// </summary>
	public class x3dTriangleStripSet : X3DNode, X3DComposedGeometryNode
	{
		...
		public List<int> StripCount { get; set; }
		...
		public x3dTriangleStripSet()
		{
			Attrib=new List<X3DVertexAttributeNode>();
			StripCount=new List<int>();
			CCW=true;
			ColorPerVertex=true;
		...
		internal override X3DPrototypeInstance GetProto() { return new x3dTriangleStripSetPROTO(); }
		...
		internal override bool ParseNodeBodyElement(string id, VRMLParser parser)
		{
			...
			else if(id=="stripCount") StripCount.AddRange(parser.ParseSFInt32OrMFInt32Value());
			else if(id=="texCoord")
			{
				X3DNode node=parser.ParseSFNodeValue();
				if(node!=null)
